...
import type { PostWithDetails } from '@/components/home/HomePage';
...
const PostCard: React.FC<PostCardProps> = ({ post, session, onViewProfile }) => {
        ...
        if (shouldShow && comments.length === 0 && commentCount > 0) {
            ...
            try {
                const { data, error } = await supabase
                    .from('comments')
                    .select('id, created_at, content, user_id, parent_comment_id, post_id, profiles(username, name, photo_url)')
                    .eq('post_id', postId)
    ...
    const handleCommentSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const commentData: TablesInsert<'comments'> = { content: newComment, post_id: postId, user_id: session.user.id };
            const { data, error } = await supabase
                .from('comments')
                .insert([commentData])
                .select('id, created_at, content, user_id, parent_comment_id, post_id, profiles(username, name, photo_url)')
