...
const EditProfilePage: React.FC<EditProfilePageProps> = ({ session, onBack, onProfileUpdated }) => {
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>, type: 'avatar' | 'cover') => {
        ...
        try {
            ...
            if (type === 'avatar') {
                await supabase.from('profiles').update({ photo_url: publicUrl } as TablesUpdate<'profiles'>).eq('id', session.user.id);
                setPhotoUrl(publicUrl);
            } else {
                await supabase.from('profiles').update({ cover_url: publicUrl } as TablesUpdate<'profiles'>).eq('id', session.user.id);
    ...
    const handleSaveChanges = async (e: React.FormEvent) => {
        ...
        try {
            const { error: updateError } = await supabase
                .from('profiles')
                .update({ name, username, bio } as TablesUpdate<'profiles'>)
                .eq('id', session.user.id);
