                    ...
                    >
                        <form onSubmit={handleSubmit}>
                            <div className="p-6 border-b border-slate-200 dark:border-slate-700">
                                <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">{productToEdit ? 'Edit Product' : 'Create New Product'}</h2>
                            </div>
                            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                                ...
                                <div>
                                    ...
                                    <textarea name="description" value={formData.description || ''} onChange={handleChange} rows={3} className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-200" />
                                </div>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">Product Type</label>
                                    <select name="product_type" value={formData.product_type} onChange={handleTypeChange} className="w-full p-2.5 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-200">
                                ...
                                {formData.product_type === 'package' ? (
                                    ...
                                    <div className="p-3 bg-slate-50 dark:bg-slate-700/50 rounded-md space-y-3">
                                        <h4 className="font-semibold text-slate-600 dark:text-slate-300">Subscription Details</h4>
                                ...
                                <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-700/50 p-3 rounded-md">
                                     <label htmlFor="is_active" className="text-sm font-medium text-slate-700 dark:text-slate-300">Product is Active</label>
                                     <button type="button" onClick={() => handleToggle('is_active', !formData.is_active)} className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors ${formData.is_active ? 'bg-violet-600' : 'bg-slate-300 dark:bg-slate-600'}`}>
