...
const Profile: React.FC<ProfileProps> = ({ session, userId, onBack, onMessage, onNavigateToSettings, onNavigateToTools, onNavigateToAdminPanel, onViewProfile }) => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            ...
            try {
                ...
                const { data: postData, error: postError } = await supabase.from('posts').select('id, content_url').eq('user_id', userId).order('created_at', { ascending: false }).returns<PostData[]>();
    ...
    const handleOpenFollowModal = async (type: 'followers' | 'following') => {
        ...
        try {
            let userIds: string[] = [];
            if (type === 'followers') {
                const { data, error } = await supabase.from('follows').select('follower_id').eq('following_id', userId).returns<{ follower_id: string }[]>();
                if (error) throw error;
                if (data) userIds = data.map(d => d.follower_id);
            } else {
                const { data, error } = await supabase.from('follows').select('following_id').eq('follower_id', userId).returns<{ following_id: string }[]>();
                if (error) throw error;
                if (data) userIds = data.map(d => d.following_id);
            }

            if (userIds.length > 0) {
                const { data: profiles, error } = await supabase.from('profiles').select('id, name, username, photo_url').in('id', userIds).returns<ProfileStub[]>();
                if (error) throw error;
        ...
        <div className="flex flex-col w-full bg-gray-50">
            ...
            <div className="p-4 transform -translate-y-12">
                <div className="flex items-end">
                    <button onClick={handleAvatarClick} className="relative focus:outline-none rounded-full focus:ring-4 focus:ring-violet-300">
                        <img src={profile.photo_url || generateAvatar(profile.name || profile.username)} alt={profile.name || 'avatar'} className="w-24 h-24 rounded-full object-cover border-4 border-white shadow-md" />
