    ...
    useEffect(() => {
        const fetchProfile = async () => {
            ...
            try {
                ...
                if ((profileData as unknown as ProfileData).xp_balance >= 10000) {
                    ...
                    if (premiumData) setPremiumFeatures(premiumData as unknown as PremiumFeatures);
                }
            } catch (error: any) {
    ...
    const handleOpenFollowModal = async (type: 'followers' | 'following') => {
        ...
        try {
            ...
            if (type === 'followers') {
                ...
                if (data) userIds = (data as unknown as { follower_id: string }[]).map(d => d.follower_id);
            } else {
                const { data, error } = await supabase.from('follows').select('following_id').eq('follower_id', profileIdToFetch);
                if (error) throw error;
                if (data) userIds = (data as unknown as { following_id: string }[]).map(d => d.following_id);
            }
            if (userIds.length > 0) {
