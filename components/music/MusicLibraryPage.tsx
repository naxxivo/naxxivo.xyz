...
const MusicLibraryPage: React.FC<MusicLibraryPageProps> = ({ session, onBack }) => {
    ...
    const handleMusicFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const { error: insertError } = await supabase.from('profile_music').insert([{ profile_id: myId, music_url: publicUrl, file_name: file.name }] as any);
    ...
    const handleGifFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const { error: insertError } = await supabase.from('profile_gifs').insert([{ user_id: myId, gif_url: publicUrl, storage_path: filePath }] as any);
            if (insertError) throw insertError;
    ...
    const handleSetActiveGif = async (gifId: number) => {
        const { error } = await supabase.from('profiles').update({ active_gif_id: gifId } as any).eq('id', myId);
        if (error) {
